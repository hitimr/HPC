...
void run_bfs(int64_t root, int64_t* pred) {
	...
	volatile uint64_t a, u, v, q1c, sum;
	queue_t q;

	// Initialize visited and predecessor list
	//Bitmap visited_bm = Bitmap_init(g.nglobalverts, false);	

	q = queue_new(100000);
	queue_enqueue(&q, root);	// Enter vertex root into queue
	
	bool* visited = (bool*) malloc(g.nglobalverts * sizeof(bool));
	//bool visited[300000];
	for(i=0; i<g.nglobalverts; i++)	visited[i] = false;
	
	pred[root] = root; //  Setting the root of the predecessor list, s as predecessor of itself
	while(!queue_empty(&q))
	{
		u = queue_dequeue(&q);
		volatile uint64_t start = rowstarts[u];
		volatile uint64_t end = rowstarts[u+1];
		for(v = start ; v < end; v++)
		{
